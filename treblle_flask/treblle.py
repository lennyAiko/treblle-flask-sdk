...
from .utils import get_json_file, mask_json_values, mask_list_values, treblle_payload, send_payload, machine_info
...
class Treblle(object):
    ...
    def init_app(self, app: Flask):
        ...
        def get_request():
            ...
            self.request_body['headers'] = mask_json_values(json.loads(json.dumps({**request.headers})), self.hidden_json_keys)
            self.request_body['timestamp'] = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        ...
        def get_response(response):
            ...
            self.response_body["headers"] = mask_json_values(headers, self.hidden_json_keys)
            body = (
                response.get_json()
